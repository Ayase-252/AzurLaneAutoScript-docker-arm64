...
RUN conda create --prefix $PYROOT python=3.7 -y \
 && apt-get update \
 && apt-get install -y libgl1 libatlas-base-dev libopencv-dev build-essential libopenblas-dev unzip \
 ...
 && conda clean --all -y && $PYROOT/bin/pip cache purge && rustup self uninstall -y && apt-get purge -y unzip \
